...
function emit(instance, event, ...rawArgs) {
  ...
  if (!handler && isModelListener) {
    handlerName = toHandlerKey(hyphenate2(event));
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  if (!(instance.type.__hmrId || instance.parent && instance.parent.type.__hmrId) && (optimized || patchFlag > 0) && !(patchFlag & 16)) {
    ...
    for (const key in rawCurrentProps) {
      if (!rawProps || !hasOwn(rawProps, key) && ((kebabKey = hyphenate2(key)) === key || !hasOwn(rawProps, kebabKey))) {
        if (options) {
...
function normalizePropsOptions(comp, appContext, asMixin = false) {
  ...
  if (isArray2(raw)) {
    ...
    for (const key in raw) {
      ...
      if (validatePropName(normalizedKey)) {
        const opt = raw[key];
        const prop = normalized[normalizedKey] = isArray2(opt) || isFunction(opt) ? {type: opt} : opt;
        if (prop) {
...
var setRef = (rawRef, oldRawRef, parentSuspense, vnode) => {
  if (isArray2(rawRef)) {
    rawRef.forEach((r, i) => setRef(r, oldRawRef && (isArray2(oldRawRef) ? oldRawRef[i] : oldRawRef), parentSuspense, vnode));
    return;
  }
  let value;
...
var toTypeString2 = (value) => objectToString.call(value);
...
var getModelAssigner = (vnode) => {
  ...
  return isArray(fn) ? (value) => invokeArrayFns2(fn, value) : fn;
};
...
//# sourceMappingURL=chunk.DOJKPUCE.js.map
